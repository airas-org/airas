...
典型的なフロー: `search_papers` → `fetch_paper_fulltext`（ファイルを読む）→ **仮説と実験設計を自分で書く** → `prepare_repository` → **実験コードを自分で書く**（下記参照）→ `dispatch_experiment` →（`get_experiment_run_status` でポーリング）→ `import_run_outputs` → `fetch_experiment_results` → `analyze_experiment` → **図とダイアグラムを自分で作る**（下記参照）→ `generate_bibfile` → `generate_paper` → `generate_latex` → **返された LaTeX を clone 内の `.research/latex/{テンプレート}/main.tex` に書いて git で push** → **出口は2つ（片方でも両方でも）**: `compile_latex`（GitHub ActionsでPDFをビルド）/ `open_in_overleaf`（ユーザーが編集できる形でOverleafに引き渡し。`local_path` 指定なら push 不要）
...
時間のかかるステップは完了を待たず、ツール自体は即座に返ります。実験は `dispatch_experiment` の `backend` に応じて GitHub Actions または Seyval 上で実行されます。どちらも `execution_id` を返し、`get_experiment_run_status` で追跡、`import_run_outputs` で回収します。LaTeX ビルドは GitHub Actions 上で実行され、`get_workflow_runs` で追跡します。
