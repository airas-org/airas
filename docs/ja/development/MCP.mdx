...
執筆ステップには 2 車線があります。skills に導かれて agent 自身が書く車線では、`get_prompts` がその step のプロンプトを返します（`hypothesis_and_design` / `experiment_code` / `experiment_analysis` / `paper_writing`、キー不要）。バックエンド LLM のツールがある step（`analyze_experiment`、`model` 付きの `verify_paper_values`。プロバイダーキーが必要）では、ツールが同じプロンプトに答えます。各プロンプトには作成すべきデータ形式を示す `output_json_schema` と、出力の行き先を示す `flow` が付きます。
