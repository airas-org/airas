...
/*
 * Documentation:
 * Accordion — https://app.subframe.com/32f8a386b602/library?component=Accordion_d2e81e20-863a-4027-826a-991d8910efd9
 * Tree View — https://app.subframe.com/32f8a386b602/library?component=Tree+View_4ed46422-ecc3-41e8-8787-e55ee10cdc75
 */
...
const Folder = React.forwardRef<React.ElementRef<typeof Accordion>, FolderProps>(function Folder(
  { children, label, icon = <FeatherFolder />, className, ...otherProps }: FolderProps,
  ref,
) {
  return (
    <Accordion
      className={SubframeUtils.twClassNames("group/c841484c cursor-pointer", className)}
      trigger={
    ...
    >
      {children ? (
        <div className="flex w-full flex-col items-start gap-1 pl-6 pt-1">{children}</div>
      ) : null}
    </Accordion>
  );
});
...
const Item = React.forwardRef<HTMLDivElement, ItemProps>(function Item(
  { selected = false, label, icon = <FeatherFile />, className, ...otherProps }: ItemProps,
  ref,
) {
  return (
    <div
...
const TreeViewRoot = React.forwardRef<HTMLDivElement, TreeViewRootProps>(function TreeViewRoot(
  { children, className, ...otherProps }: TreeViewRootProps,
  ref,
) {
  return children ? (
    <div
      className={SubframeUtils.twClassNames("flex w-full flex-col items-start", className)}
      ref={ref}
      {...otherProps}
    >
      {children}
