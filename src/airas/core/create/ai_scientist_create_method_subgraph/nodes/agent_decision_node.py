...
from airas.create.create_method_subgraph.create_method_subgraphv2 import CreateMethodSubgraphState
...
def agent_decision_node(
        ...
        if verification_result.get("is_novel", False) and confidence_score >= self.novelty_threshold:
            print(f"✅ HARD CONSTRAINT: Novel method with high confidence ({confidence_score:.2f} >= {self.novelty_threshold})")
            return "finalize"
        ...
        try:
            ...
            agent_prompt = PromptManager.render_prompt(
                PromptManager.get_agent_decision_prompt,
                **template_data
            )
            ...
            print(f"🔑 Key factors: {', '.join(key_factors[:3])}...")  # Show first 3 factors
