...
def _write_tables(latex_dir: Path) -> None:
    ...
    tables_dir.mkdir()
    (tables_dir / f"{TABLE.key}.tex").write_text(render_table_tex(TABLE, METRICS_DATA))


def test_verify_tables_passes_fresh_generation(tmp_path: Path) -> None:
    _write_tables(tmp_path)
    assert _verify_tables(tmp_path, [TABLE], METRICS_DATA) == []
...
def test_verify_tables_detects_edited_cell(tmp_path: Path) -> None:
    ...
    problems = _verify_tables(tmp_path, [TABLE], METRICS_DATA)
...
def test_verify_tables_reports_missing_declared_table(tmp_path: Path) -> None:
    problems = _verify_tables(tmp_path, [TABLE], METRICS_DATA)
    assert any("main_results.tex is missing" in p for p in problems)
...
def _write_chart(
    ...
    record = _chart_record(
        ChartDeclaration(path=relative, format="svg", spec=CHART_SPEC)
    )
    return chart_path, record
...
def test_verify_charts_detects_redirected_declaration(tmp_path: Path) -> None:
    # Point the declaration at the other run: the re-render no longer
    ...
    tampered_spec = {
        **CHART_SPEC,
        "data": {
            "values": [
                {"method": "Ours", "acc": "metric:run_1.accuracy"},
                {"method": "Baseline", "acc": "metric:run_1.accuracy"},
            ]
        },
    }
...
def test_verify_charts_rejects_undeclared_chart_in_subdirectory(
    tmp_path: Path,
) -> None:
    # The LaTeX export collects PDFs recursively, so nesting must not
    # evade the declaration requirement.
...
def test_verify_charts_reports_missing_declared_chart(tmp_path: Path) -> None:
    record = _chart_record(
...
def test_png_charts_render_and_verify(tmp_path: Path) -> None:
    chart_path = tmp_path / CHART_DIR / "accuracy.png"
