...
from airas.research_record.verify._verify_results_against_store import (
    provenance_problems as _provenance_problems,
)
from airas.research_record.verify._verify_results_against_store import (
...
async def _verify(fake: FakeSeyvalClient, path: str, dirs: set[str]):
    return await verify_results_against_store(
...
async def test_a_dir_is_checked_through_its_declared_backend(tmp_path: Path) -> None:
    ...
    result = await verify_results_against_store(str(tmp_path), {"run-1"}, factory)
    assert result.status == "verified", result.checks[0].detail
...
async def test_an_expired_store_falls_back_to_the_import_time_hashes(
    ...
    result = await verify_results_against_store(
        str(tmp_path), {"run-1"}, lambda b, u: store
    )
    assert result.status == "verified", result.checks[0].detail
