...
def get_model_context_info(model_name: str) -> dict[str, int]:
    ...
    # litellm may report an explicit None; fall back to 4096 only in that case.
    max_input = info.get("max_input_tokens")
    max_output = info.get("max_output_tokens")
    return {
        "max_input_tokens": 4096 if max_input is None else max_input,
        "max_output_tokens": 4096 if max_output is None else max_output,
    }
