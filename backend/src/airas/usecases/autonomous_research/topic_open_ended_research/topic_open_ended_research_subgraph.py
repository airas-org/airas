...
class TopicOpenEndedResearchSubgraph:
    ...
    async def _process_main_experiment_branch(
        ...
        # Derive run_id from the branch name and the base branch prefix.
        # Reading state["run_id"] directly is unsafe: run_id is not a schema-defined
        # channel, so LangGraph drops it from state after the first node in the retry
        # loop returns, causing KeyError on subsequent iterations.
