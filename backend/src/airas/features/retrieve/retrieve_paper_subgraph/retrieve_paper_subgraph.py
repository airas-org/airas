...
from airas.features.retrieve.retrieve_code_subgraph.node.extract_github_url_from_text import (
    extract_github_urls_from_text_groups,
)
...
class RetrievePaperSubgraph:
    ...
    async def _summarize_paper(
        ...
        arxiv_summary_list = await summarize_paper(
        ...
        return {"arxiv_summary_list": arxiv_summary_list}

    @record_execution_time
    async def _extract_github_url_from_text(
        self, state: RetrievePaperSubgraphState
    ) -> RetrievePaperSubgraphState:
        github_url_list = extract_github_urls_from_text_groups(
            arxiv_full_text_list=state.get("arxiv_full_text_list", []),
    ...
    def build_graph(self):
        ...
        graph_builder.add_edge("summarize_paper", "extract_github_url_from_text")
        graph_builder.add_edge(
            "extract_github_url_from_text", "retrieve_repository_contents"
        )
        graph_builder.add_edge("retrieve_repository_contents", END)
