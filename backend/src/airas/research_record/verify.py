...
from airas.research_record.passages import quote_in
...
from airas.research_record.run_provenance import (
    StoreFactory,
...
async def verify_record(
    ...
    *,
    ...
    problems = await asyncio.to_thread(verify_consistency, record)
    problems += await asyncio.to_thread(verify_literature, root, record)
    problems += await asyncio.to_thread(
        verify_append_only, root, record, require_history
    )
...
# ------------------------------------------------------ the record alone
def verify_consistency(record: ResearchRecord) -> list[str]:
    problems: list[str] = []
    ...
    return problems + verify_pinned_once(record) + verify_passage_references(record)


def verify_pinned_once(record: ResearchRecord) -> list[str]:
...
def verify_passage_references(record: ResearchRecord) -> list[str]:
...
# --------------------------------------------------------- the literature
def verify_literature(root: Path, record: ResearchRecord) -> list[str]:
...
def verify_append_only(
    ...
    problems = [
        ...
        for problem in verify_passage_references(version)
    ]
